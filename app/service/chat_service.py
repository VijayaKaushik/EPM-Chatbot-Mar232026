...
def exec_chat(request: ChatRequest) -> ChatResponse:
    ai_response = run_query(
        query=request.user_msg, session_id=request.session_id, user_id=request.user_id
    )
    return ChatResponse(
...
async def get_all_messages(request: ChatHistoryRequest):
    ai_user_history = await get_ai_user_chat_history(
    ...
    for (
        role,
        message,
    ) in ai_user_history:  ## converting list of tuple to list of ChatMessage object
